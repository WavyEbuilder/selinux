...
{
	if (__selinux_key_create(&destructor_key, procattr_thread_destructor) ==
	    0) {
...
static int openattr(pid_t pid, const char *attr, int flags)
{
	...
	static_assert(sizeof(pid_t) <= sizeof(uint32_t),
		      "content written to path might get truncated");

	if (pid > 0) {
		rc = snprintf(path, sizeof(path), "/proc/%d/attr/%s", pid,
			      attr);
	} else if (pid == 0) {
		rc = snprintf(path, sizeof(path), "/proc/thread-self/attr/%s",
			      attr);
		...
		if (fd >= 0 || errno != ENOENT)
			return fd;
		tid = selinux_gettid();
		rc = snprintf(path, sizeof(path), "/proc/self/task/%d/attr/%s",
			      tid, attr);
	} else {
		errno = EINVAL;
		return -1;
...
all_selfattr_def(con, "current", prev_current)
	getselfattr_def(prevcon, "prev", NULL) all_selfattr_def(execcon, "exec",
								prev_exec)
