...
static int dbus_server(GMainLoop *loop)
{
	...
	client_id = g_bus_own_name_on_connection(bus, DBUS_NAME,
...
static gboolean io_channel_callback(GIOChannel *source, GIOCondition condition,
...
int start(void)
{
	...
	result = g_dbus_connection_call_sync(
		bus, DBUS_NAME, /* bus name */
		"/", /* object path */
		"org.freedesktop.DBus.Peer", /* interface */
		"Ping", /* method */
		NULL, /* parameters */
		NULL, /* reply_type */
		G_DBUS_CALL_FLAGS_NONE, -1, /* timeout_msec */
		NULL, &err);
	if (!result) {
		g_object_unref(bus);
		if (debug_mode)
...
static int local_server(void)
{
	...
	g_io_channel_set_flags(
		in, g_io_channel_get_flags(in) | G_IO_FLAG_NONBLOCK, NULL);
	g_io_add_watch_full(in, G_PRIORITY_HIGH, G_IO_IN | G_IO_ERR | G_IO_HUP,
...
int server(int master_fd, const char *watch_file)
{
	...
	g_io_channel_set_flags(
		c, g_io_channel_get_flags(c) | G_IO_FLAG_NONBLOCK, NULL);

	g_io_add_watch_full(c, G_PRIORITY_HIGH, G_IO_IN | G_IO_ERR | G_IO_HUP,
			    io_channel_callback, NULL, NULL);

	/* Handle SIGTERM */
	g_unix_signal_add_full(G_PRIORITY_DEFAULT, SIGTERM, sigterm_handler,
			       loop, NULL);
