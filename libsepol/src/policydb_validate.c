			       ...
			       validate_t flavors[], const policydb_t *p)
{
	...
	if (p->mls && p->policy_type != POLICY_MOD &&
	    validate_mls_range(&user->exp_range, &flavors[SYM_LEVELS],
			       &flavors[SYM_CATS]))
		goto bad;
	if (p->mls && p->policy_type != POLICY_MOD &&
					...
					validate_t flavors[])
{
	...
	if (hashtab_map(p->p_classes.table, validate_class_datum_wrapper,
			&margs))
		goto bad;
...
static int validate_symtabs(sepol_handle_t *handle, const symtab_t symtabs[],
			    validate_t flavors[])
{
	unsigned int i;

	for (i = 0; i < SYM_NUM; i++) {
		if (hashtab_map(symtabs[i].table, validate_datum,
				&flavors[i].nprim)) {
