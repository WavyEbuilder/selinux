...
__attribute__((format(printf, 2, 3))) void sepol_printf(FILE *out,
							const char *fmt, ...);

__attribute__((format(printf, 1, 2))) char *create_str(const char *fmt, ...);
...
strs_create_and_add(struct strs *strs, const char *fmt, ...);
