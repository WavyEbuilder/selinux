...
#define TYPE_FLAGS_EXPAND_ATTR \
	(TYPE_FLAGS_EXPAND_ATTR_TRUE | TYPE_FLAGS_EXPAND_ATTR_FALSE)
...
extern int policydb_from_image(sepol_handle_t *handle, void *data, size_t len,
			       policydb_t *policydb);

extern int policydb_to_image(sepol_handle_t *handle, policydb_t *policydb,
...
extern int policydb_role_cache(hashtab_key_t key, hashtab_datum_t datum,
			       void *arg);

extern int policydb_user_cache(hashtab_key_t key, hashtab_datum_t datum,
			       void *arg);
...
#define POLICYDB_VERSION_COND_XPERMS \
...
#define POLICYDB_CONFIG_UNKNOWN_MASK \
	(DENY_UNKNOWN | REJECT_UNKNOWN | ALLOW_UNKNOWN)
